...
# （用于测试推送如果改了能收到推送，推送设置就没问题，看看是不是set_push列表里面没设置推送，仔细看下面我写的很详细）要修改的步数，直接输入想要修改的步数值，
#（默认）留空为随机步数，改了这个直接运行固定值（用于测试推送）
# 测试好记得留空不然一直提交固定步数
step1 = ""
...
min_dict = {time_list[0]: 2000, time_list[1]: 3000, time_list[2]: 2000, time_list[3]: 6000, time_list[4]: 7000, time_list[5]: 0, time_list[6]: 0}
# 最大步数（例如现在设置意思是在8点（你设置的第一个时间点默认8）运行会在1500到2999中随机生成一个数提交（开启气候降低步数会乘系数K）10点3000~4999。。。以此类推，步数范围建议看懂了再改，没看懂直接默认就好）
max_dict = {time_list[0]: 2999, time_list[1]: 4999, time_list[2]: 5999, time_list[3]: 6999, time_list[4]: 8999, time_list[5]: 0, time_list[6]: 0}
# 设置结束
...
def main(_user,_passwd,min_1, max_1):
    ...
    global push_title
    push_title = f"账号{user[7:]}修改步数（{step}）"
    #print(result)
    return result
